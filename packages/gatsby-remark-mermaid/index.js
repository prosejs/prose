...
const plugin = async ({ markdownAST }, options) => {
  ...
  const defaultPostRender = (value) => {
    ...
    // replace namespaces
    val = val.replace(/xmlns=".*?"/g, '')
    val = val.replace(/xmlns:xlink=".*?"/g, '')

    // TODO: review converting HTML to JSX options
    // replace max-width with maxWidth
    val = val.replace(/max-width/g, 'maxWidth')

    // replace white-space with whiteSpace
    val = val.replace(/white-space/g, 'whiteSpace')
